...
use crate::{core::dep::Dep, model::user::Credentials, pkg::util::create_auth_response};
...
pub async fn sign(
    ...
    let token = dep.jwt_encode(credentials.username.clone())?;
...
pub async fn login(
    ...
    let token = dep.jwt_encode(user.username.clone())?;
